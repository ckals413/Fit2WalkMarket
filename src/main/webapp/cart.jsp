			...
			<table class="table table-hover">
				...
				<%				
					...
					for (int i = 0; i < cartList.size(); i++) { // 상품리스트 하나씩 출력하기
						...
						int total = shoes.getUnitPrice() * shoes.getQuantity();
						sum = sum + total;
				%>
				<tr>
					<td><%=shoes.getProductId()%> - <%=shoes.getName()%></td>
					<td><%=shoes.getUnitPrice()%></td>
					<td><%=shoes.getQuantity()%></td>
					<td><%=total%></td>
					<td><a href="./removeCart.jsp?id=<%=shoes.getProductId()%>" class="badge text-bg-danger">삭제</a></td>
				</tr>
				<%
					}
				%>
